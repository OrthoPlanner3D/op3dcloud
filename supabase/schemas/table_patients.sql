create table op3dcloud.patients (
  id bigint generated always as identity not null,
  id_client uuid not null,
  name text not null,
  last_name text not null,
  type_of_plan text not null,
  treatment_approach text not null,
  treatment_objective text[] not null default '{}'::text[],
  dental_restrictions text[] not null default '{}'::text[],
  declared_limitations text[] not null default '{}'::text[],
  planning_enabled boolean not null default false,
  suggested_adminations_and_actions text[] not null default '{}'::text[],
  observations_or_instructions text not null,
  files text[] not null,
  sworn_declaration boolean not null default false,
  expiration date null,
  id_planner uuid null,
  status_files text[] null,
  case_status text[] null,
  notes text null,
  observations text null,
  status text null,
  photos text[] not null,
  xrays text[] not null,
  scans text[] not null,
  supplementary_docs text[] null,
  created_at timestamp with time zone not null default now(),
  constraint patients_pkey primary key (id),
  constraint patients_id_client_fkey foreign key (id_client) references auth.users (id),
  constraint patients_id_planner_fkey foreign KEY (id_planner) references auth.users (id)
);

alter table op3dcloud.patients enable row level security;

create policy "CRUD"
on op3dcloud.patients for all
to authenticated
using(true)
with check (true);
